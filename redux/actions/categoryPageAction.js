...
export const categoryPostUpdateGETAction = (cat, limit) => {
  return { type: "GET_CATEGORY_UPDATE_POST", cat, limit };
};

export function* watchCategoryPostGET() {
  yield takeEvery("GET_CATEGORY_POST", workerCategoryPostGET);
  yield takeEvery("GET_CATEGORY_UPDATE_POST", workerCategoryUpdatePostGET);
}

export function* workerCategoryPostGET({ cat, limit }) {
  try {
    yield put({ type: constant.CATEGORY_POST_GET_INITIATE, payload: [] });
...
export function* workerCategoryUpdatePostGET({ cat, limit }) {
  try {
    yield put({
      type: constant.CATEGORY_POST_UPDATE_GET_INITIATE,
    });
